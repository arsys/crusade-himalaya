<section class=" uk-margin-medium-top uk-margin-medium-bottom bgcolor-wrapper">
    ...
    <div class="uk-container-expand">
        ...
        <div class="follow uk-grid-collapse uk-child-width-expand@s uk-child-width-1-2   uk-child-height-1-2 " uk-grid="">
            @foreach($row as $feed)
            <div><a href="#modal-media-image-{{ $loop->iteration }}" uk-toggle><img src="{{ $feed->thumb_link }}" alt="{{ $feed->caption }}" ></a></div>
            @endforeach
        </div>
        @endforeach
...
{{--         <div class="follow uk-grid-collapse uk-child-width-expand@s uk-child-width-1-2   uk-child-height-1-2  " uk-grid="">
        ...
        <div id="modal-media-image-{{  }}" class="uk-flex-top" uk-modal>
            ...
            <a class="uk-position-center-left uk-position-small uk-slidenav-large previous" href="#modal-media-image-{{$loop->iteration-1 }}" uk-slidenav-previous uk-toggle></a>
            @endif
            @if($loop->iteration != $loo->last)
            <a class="uk-position-center-right uk-position-small  uk-slidenav-large next" href="#modal-media-image-{{ $loop->iteration+1 }}" uk-slidenav-next uk-toggle></a>
            @endif
        </div>
        @endforeach
