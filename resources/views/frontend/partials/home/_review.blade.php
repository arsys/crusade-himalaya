
<div class="uk-container uk-margin-medium-top uk-margin-mediumm-bottom">
    <div class="uk-grid-divider " uk-grid >
        <div class="uk-width-1-2@m">
            <h2 class=" uk-text-center strike uk-margin-medium-bottom"><span>Our Approach</span></h2>
            <p class="uk-text-center uk-margin-remove-top home-heading-secondary uk-margin-large-bottom  " id="home-sub-title"><span>Corporate Social Responsibility</span></p>
            <p> We are here to make a positive contribution to society.
                We believe being socially responsible & giving back is our way of life.
                It's Time Elevate local communities to education & their welfare.
                Small effort Towards the community can make a big difference,
                after all priceless smiles is all that counts.
            </p>
        </div>
        <div class="uk-width-1-2@m">
            <img data-src="{{ asset('img/1.jpg') }}" width="576" height="486" alt="Crusade Himalay in Donation Program" uk-img>
        </div>
    </div>
</div>
