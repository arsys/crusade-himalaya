<div class="uk-grid-divider uk-child-width-expand@s uk-margin-medium-bottom text-color include-wrapper" uk-grid>
    @if (!empty($tour->includes) && !empty($tour->excludes) && !empty($tour->budgetIncludes) && !empty($tour->budgetExcludes))
    <div>
    ...
    <div>
        ...
        <ul class="uk-list includes">
            @foreach($tour->excludes as $exclude)
            <li><span class="uk-margin-small-right included" uk-icon="check"></span>{{ $exclude->name }}</li>
            @endforeach
        </ul>
